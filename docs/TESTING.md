...
## 4. 필수 엣지 케이스 체크리스트 (component — 파이프라인)

**입력·추출**
- [ ] PDF/DOCX/TXT 각 1건 정상 경로 (실 추출기 + 픽스처 파일)
- [ ] 빈 텍스트 PDF(스캔본) → "텍스트를 추출할 수 없음(스캔본은 v0.2 OCR 예정)" 모국어 안내
- [ ] 암호 PDF → 명확한 안내, 크래시 없음
- [ ] 20MB 초과 메타 → 다운로드 시도 없이 reject

**분할·조립**
- [ ] 섹션 경계 우선 분할, 청크 순서 보존 조립 (마커로 검증)
- [ ] 청크 1개 실패 주입 → 1회 재시도 → 성공 시 완성 / 재실패 시 부분 실패 안내 + 완역 미게시
...
**게시·프라이버시**
- [ ] 4,096자 초과 전문 → 문단 경계 분할 다중 postText, 순서 보장
- [ ] 게시 대상 chatId = 수신 chatId 고정 (다른 chatId 게시 시도 시 테스트 실패)
- [ ] 처리 종료 후 임시 버퍼·파일 잔류 없음 / 로그 캡처에 본문 문자열 미포함 (본문 시그니처 문자열로 검사)
- [ ] FakeTranslator 호출 수 ≤ 청크 수 + 요약 1 (중복 호출 = 비용 누수 가드)

**동시성**
- [ ] 서로 다른 chat 2건 동시 업로드 → 진행 메시지·결과 혼입 없음
